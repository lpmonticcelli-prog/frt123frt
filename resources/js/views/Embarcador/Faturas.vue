<template>
  <div class="space-y-6 relative">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div v-else-if="faturas.length === 0" class="p-16 text-center">
        ...
        <p class="text-sm text-slate-500 mt-1">Sua infraestrutura logística ainda não gerou cobranças no ciclo atual.</p>
      </div>
      ...
      <table v-else class="min-w-full divide-y divide-gray-200 text-left">
        <thead class="bg-slate-50">
          <tr>
            ...
            <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Emissão</th>
        ...
        <tbody class="bg-white divide-y divide-gray-100">
          <tr v-for="fatura in faturas" :key="fatura.id" class="hover:bg-slate-50/50 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap text-sm font-black text-slate-900 font-mono">Comp. {{ fatura.mes_referencia }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-700">{{ formatarData(fatura.created_at) }}</td>
            <td class="px-6 py-4 whitespace-nowrap">
              <div :class="['text-sm font-bold', isVencida(fatura) ? 'text-red-600' : 'text-slate-900']">
                {{ formatarData(fatura.data_vencimento) }}
              </div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-black text-slate-900">{{ formatarMoeda(fatura.valor_total) }}</td>
            <td class="px-6 py-4 whitespace-nowrap">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right space-x-3">
              <button @click="abrirDetalhes(fatura.id)" class="text-slate-600 hover:text-slate-900 font-bold transition-colors text-sm">
                Detalhes
              </button>
              <button v-if="fatura.status === 'pendente' || fatura.status === 'vencida'" @click="abrirPagamento(fatura)" class="inline-flex items-center px-4 py-1.5 bg-slate-900 text-white text-xs font-bold rounded shadow-sm hover:bg-slate-800 transition-colors">
                Pagar Agora
              </button>
              <a v-if="fatura.status === 'paga' && fatura.link_boleto" :href="fatura.link_boleto" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-green-50 border border-green-200 text-green-700 font-bold text-xs rounded hover:bg-green-100 transition-colors">
                Baixar Recibo
              </a>
            </td>
          </tr>
      ...
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl w-full border border-slate-200">
          <div class="bg-white px-6 pt-6 pb-6 sm:p-8">
            <div class="flex justify-between items-start border-b border-slate-100 pb-4 mb-6">
              <div>
                ...
                <p class="text-sm text-slate-500 font-mono mt-1">Competência: {{ faturaSelecionada?.mes_referencia || 'Carregando...' }}</p>
              </div>
            ...
            <div v-if="carregandoDetalhes" class="py-12 text-center text-slate-500 font-medium animate-pulse">
                ...
                <table class="min-w-full divide-y divide-slate-100 text-sm">
                  ...
                  <tbody class="divide-y divide-slate-100">
                    <tr v-for="carga in faturaSelecionada.detalhes_cargas?.cargas || []" :key="carga.id" class="hover:bg-slate-50">
                      ...
                      <td class="px-4 py-3 text-slate-600">{{ carga.rota || 'Rota Desconhecida' }}</td>
          ...
          <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse border-t border-slate-200">
            <button @click="fecharModalDetalhes" class="px-6 py-2 bg-white border border-slate-300 rounded-lg text-sm font-bold text-slate-700 hover:bg-slate-100">Fechar</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md w-full border border-slate-200">
          <div class="bg-white px-6 pt-6 pb-8 text-center">
            ...
            <p class="text-sm text-slate-500 mb-6">Competência {{ faturaPagamento?.mes_referencia }} • Valor: <strong class="text-slate-900">{{ formatarMoeda(faturaPagamento?.valor_total) }}</strong></p>
          ...
          <div class="bg-slate-50 px-6 py-4 flex border-t border-slate-200">
            <button @click="fecharModalPagamento" class="w-full text-center text-sm font-bold text-slate-600 hover:text-slate-900 transition-colors">Cancelar e Voltar</button>
          </div>
        </div>
      </div>
    </div>
...
const getStatusClass = (f) => {
  if (!f) return '';
  if (f.status === 'paga') return 'bg-green-50 text-green-700 border-green-200';
