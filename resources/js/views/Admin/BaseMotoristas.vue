    ...
    <div v-else class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in">
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Motorista / CNH</th>
        ...
        <tbody class="divide-y divide-gray-200">
          ...
          <tr v-for="user in motoristas" :key="user.id" class="hover:bg-gray-50" :class="{'opacity-50 bg-red-50': user.status === 'banned'}">
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              <div class="text-xs text-gray-500">CPF: {{ user.motorista?.cpf || 'Não preenchido' }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
...
const alterarStatus = async (user, status) => {
  ...
  try {
    // Reutiliza o endpoint existente de alteração de status que já temos no AdminController
    await axios.post(`/api/admin/usuarios/${user.id}/status`, { status });
    await carregarMotoristas(); // Atualiza a tabela
  } catch (error) {
